// ====== Configuration ======
const N8N_WEBHOOK_URL = "http://localhost:5678/webhook/d217914a-4cca-4049-bd01-0759581d687b" // e.g., "https://your-n8n-url/webhook/insights"
const N8N_TRACKING_URL = "" // e.g., "https://your-n8n-url/webhook/track"
const N8N_TASK_URL = "" // optional extra for Create Task

// ====== Supabase Configuration ======
const SUPABASE_URL = 'https://axicogqtiindhiehfbuz.supabase.co'  // Get from Supabase dashboard
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImF4aWNvZ3F0aWluZGhpZWhmYnV6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTk1NjE1NjgsImV4cCI6MjA3NTEzNzU2OH0.MoOTFZ0viqkWr_s7vDIzBgMzrx32LW_PTAWKRwCQNYQ' // Get from Supabase dashboard
...
// Save startup to Supabase
// Enhanced save function with full card data
...
// Remove startup from Supabase
async function removeStartupFromDB(startup) {
  try {
    const { error } = await supabaseClient
      .from('saved_startups')
      .delete()
    ...
    if (error) throw error;
    console.log("🗑️ Removed from Supabase:", startup);
  } catch (error) {
    console.error("❌ Remove from DB failed:", error);
  }
}

// Load saved startups from Supabase
...
function persist() {
  ...
  localStorage.setItem("ip_saved", JSON.stringify(state.savedStartups)); // Keep as backup
...
function readAuth() {
  try {
    ...
    if (auth) {
      // Handle both old and new auth formats
      state.userId = auth.userId || auth.email; // This will now work
      state.auth = auth;
...
function requireAuth() {
  ...
  // Set userId for database operations
  state.userId = auth.userId || auth.email;
  return true;
}

// ====== Manual Save Functionality ======
// Enhanced save function with DB support
// Enhanced save function with full card data
...
function upsertSavedFromResults() {
  // Don't automatically save anything - manual saving only
  // This function is kept for future use but does nothing now
}
...
function removeFromHistory(query) {
  ...
  // Update the dropdown with current filter
...
function renderResults() {
  ...
  state.filtered.forEach((item) => {
    ...
    card.innerHTML = `
     ...
     onerror="this.src='https://via.placeholder.com/32/9CA3AF/FFFFFF?text=${escapeAttr(item.startup.charAt(0).toUpperCase())}'" />
        ...
        <div class="flex items-center gap-2">
          ...
          <button class="save-btn rounded p-1 ${isSaved ? 'text-yellow-500' : 'text-gray-600'} hover:bg-gray-100" title="${isSaved ? 'Remove from Saved' : 'Save Startup'}" aria-label="${isSaved ? 'Remove from Saved' : 'Save Startup'}">${isSaved ? '★' : '☆'}</button>
          <button class="bookmark-btn rounded p-1 text-gray-600 hover:bg-gray-100" title="Bookmark" aria-label="Bookmark">⭐</button>
          <button class="share-btn rounded p-1 text-gray-600 hover:bg-gray-100" title="Share" aria-label="Share">📤</button>
        </div>
      </div>
      <div class="mt-2">
    ...
    e.stopPropagation();
    // Create full startup data object
    const startupData = {
...
function renderSaved() {
  try {
    ...
    // Check if we have any saved startups
    ...
    state.savedStartups.forEach((startup) => {
      ...
      card.querySelector(".remove-btn").addEventListener("click", (e) => {
        e.stopPropagation();
        toggleSaveStartup(startup); // Use original startup object for removal
      });
...
// Helper function to process each company without duplication
function processCompanyInsights(company, outputArray) {
  ...
  // Process each insight for this company
  (company.insights || []).forEach((insight, index) => {
    // Create unique insight object
    const uniqueInsight = {
      startup: startup,
      logo: logo,
...
function generateLogoUrl(startupName) {
  ...
  return logoUrls[0]; // Use first option, add error handling in img tag
}
...
async function createTask(item) {
  try {
    if (N8N_TASK_URL) {
      await fetch(N8N_TASK_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type: "create_task", item }),
      })
      toast("Task created via webhook")
    } else {
      console.log("[Create Task]", item)
      toast("Task created (demo)")
    }
  } catch (e) {
    console.error(e)
